...
function clampUtf16WithoutSplittingSurrogate(value: string, maxChars: number): string {
...
export function boundedProviderDiagnostic(value: string | undefined): string | undefined {
  if (value === undefined) return undefined
  if (Buffer.byteLength(value, 'utf8') <= MAX_PROVIDER_DIAGNOSTIC_BYTES) return value

  let bytes = 0
  let bounded = ''
  for (const character of value) {
    const characterBytes = Buffer.byteLength(character, 'utf8')
    if (bytes + characterBytes > MAX_PROVIDER_DIAGNOSTIC_BYTES) break
    bounded += character
...
export function describeAbnormalResult(result: SubagentResult): string {
  let headline: string
  ...
  const partial = clampUtf16WithoutSplittingSurrogate(
    textFromContentBlocks(result.output),
 ...
 */
export async function settlePublishedCodexRun(run: SubagentRun): Promise<CodexRunSettlement> {
...
export function classifyPublishedSettlement(
  settlement: Awaited<ReturnType<typeof settlePublishedCodexRun>>,
  signal: AbortSignal,
): SubagentResult {
  ...
  if (execution.status === 'rejected') {
    if (signal.aborted && !(execution.reason instanceof AggregateError)) {
      throw new ContextDelegationError(
        'CODEX_CANCELLED',
        'Codex delegation was cancelled while awaiting its result.',
        {
          layer: 'delegation',
          codexInvoked: true,
          workspaceMayHaveChanged: true,
    ...
    throw new ContextDelegationError(
      'CODEX_DELEGATION_FAILED',
      'Codex run failed with an unrepresentable infrastructure error.',
      {
        layer: 'delegation',
        codexInvoked: true,
        workspaceMayHaveChanged: true,
