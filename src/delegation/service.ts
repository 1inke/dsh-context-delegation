...
import { loadContextCandidates, loadPersistentContext } from '../context/loader.ts'
import { selectContext } from '../context/selection.ts'
import { buildContextPreview } from '../context/preview.ts'
import { validateRelevantFiles } from '../context/path-confinement.ts'
import { buildDelegationHandoff } from '../handoff/builder.ts'
import { ContinuationTransportAdapter, type ContinuationTurnResult } from './transport.ts'
import { DelegationSessionRegistry, type SessionAcquisitionTurn } from './session.ts'
import { buildContextSnapshot, computePolicyRevision } from '../context/fingerprint.ts'
import { selectContinuationPayload, extractContextAck } from '../handoff/protocol.ts'
import { finishLoopTrace as finishDelegationTrace, hasReviewRequest, runReviewedDelegation } from '../verification/review-loop.ts'
import { startDelegationTrace } from '../observability/trace.ts'
import { SubagentExecutor } from '../executors/subagent.ts'
import type {
  DelegationInput,
  DelegationResult,
  ContextDelegationConfig,
  ContextDelegationServiceDescription,
  ContextLoadResult,
  ContextPreview,
  DelegationExecution,
...
export class ContextDelegationService extends Service {
  ...
  private readonly transport: ContinuationTransportAdapter
  private readonly sessionRegistry: DelegationSessionRegistry
  ...
  constructor(ctx: Context, config: ContextDelegationConfig = {}) {
    ...
    this.ctx.logger.info(
      `dsh-context-delegation contract v1 loaded (provider=${this.config.providerName}, `
  ...
  async prepareContext(
    ...
    if (!workspaceRoot || typeof workspaceRoot !== 'string' || workspaceRoot.trim() === '') {
      throw new ContextDelegationError(
        'WORKSPACE_NOT_FOUND',
        'Calling agent session does not have an authoritative working directory (cwd).',
        {
          layer: 'workspace',
          codexInvoked: false,
  ...
  async preview(input: DelegationInput, execution: DelegationExecution): Promise<ContextPreview> {
    const isContinue = input.mode === 'continue'
    if (isContinue) {
      if (!this.config.continuationEnabled) {
        throw new ContextDelegationError(
          'UNSUPPORTED_CONTINUATION_PROVIDER',
          'Continuation is disabled by configuration (continuationEnabled: false).',
          {
            layer: 'provider',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
      ...
      if (!input.delegation_key) {
        throw new ContextDelegationError(
          ...
          'delegation_key is required when mode is continue.',
          {
            layer: 'plugin',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
    ...
    const sessionCheck = execution.parent
      ? this.sessionRegistry.checkSessionCompatibility(
          execution.parent,
          input.delegation_key!,
          provider,
          compatibilityKey,
        )
      : { canReuse: false }
  ...
  async delegate(input: DelegationInput, execution: DelegationExecution): Promise<DelegationResult> {
    ...
    if (input.mode === 'continue') {
      if (hasReviewRequest(input)) {
        throw new ContextDelegationError(
          'INVALID_INPUT',
          'acceptance_criteria with mode "continue" is not supported: the reviewed flow only runs on fresh delegations.',
          {
            layer: 'delegation',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
  ...
  private async runFreshAttempt(
    input: DelegationInput,
    execution: DelegationExecution,
    trace?: import('../observability/trace.ts').DelegationTraceRecorder,
  ...
  private async delegateContinuation(
    input: DelegationInput,
    execution: DelegationExecution,
  ): Promise<DelegationResult> {
    ...
    if (typeof (providerObj as any).prepareContinuable !== 'function') {
      throw new ContextDelegationError(
        'UNSUPPORTED_CONTINUATION_PROVIDER',
        `Provider "${providerName}" does not support continuable children (lacks prepareContinuable capability).`,
        {
          layer: 'provider',
          codexInvoked: false,
          workspaceMayHaveChanged: false,
    ...
    let selectedHandoff: ReturnType<typeof selectContinuationPayload>
    let turnHandle: SessionAcquisitionTurn
    let awaitSettlement: () => Promise<ContinuationTurnResult>
    ...
    let turnResult: ContinuationTurnResult
    try {
    ...
    if (this.config.requireContextAck) {
      if (!ack.valid) {
        ...
        throw new ContextDelegationError(
          'CONTEXT_ACK_FAILED',
          `Continuable turn completed but context revision acknowledgment failed: ${ack.error ?? 'INVALID_ACK'}.`,
          {
            layer: 'context',
            codexInvoked: true,
      ...
      if (ack.acknowledgedRevision !== currentSnapshot.revision) {
        ...
        throw new ContextDelegationError(
          ...
          `Continuable child acknowledged revision "${ack.acknowledgedRevision}", but expected revision "${currentSnapshot.revision}".`,
          {
            layer: 'context',
            codexInvoked: true,
            workspaceMayHaveChanged: true,
...
function cancelledBeforeInvocation(): ContextDelegationError {
  return new ContextDelegationError(
    'CODEX_CANCELLED',
    'Delegation was cancelled before Codex was invoked.',
    {
      layer: 'delegation',
      codexInvoked: false,
      workspaceMayHaveChanged: false,
...
function providerNotFound(
  ...
  return new ContextDelegationError(
    ...
    `No subagent provider is registered as "${providerName}".`,
    {
      layer: 'provider',
      codexInvoked,
      workspaceMayHaveChanged: false,
