...
import { boundedProviderDiagnostic, classifyPublishedSettlement, settlePublishedCodexRun, textFromContentBlocks, } from "../delegation-lifecycle.js";
...
export class SubagentExecutor {
    ...
    async execute(request, execution) {
        ...
        if (!this.available()) {
            throw new ContextDelegationError('CODEX_PROVIDER_NOT_FOUND', `No subagent provider is registered as "${this.provider}".`, {
                layer: 'provider',
                codexInvoked: false,
        ...
        try {
            ...
            if (signal.aborted && !(error instanceof AggregateError)) {
                throw new ContextDelegationError('CODEX_CANCELLED', `The "${this.provider}" executor was cancelled while starting.`, {
                    layer: 'delegation',
                    codexInvoked: true,
