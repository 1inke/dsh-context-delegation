import { diffContextSnapshots } from "../context/delta.js";
...
export function buildContinuationHandoff(options) {
    ...
    if (contextMode === 'delta') {
        if (!delta) {
            throw new ContextDelegationError('PLUGIN_INTERNAL_ERROR', 'Delta object is required when contextMode is delta.', {
                layer: 'plugin',
                codexInvoked: false,
...
export function extractContextAck(text) {
...
export function parseContinuationHandoff(text) {
    const beginIndex = text.indexOf(HANDOFF_PAYLOAD_V3_BEGIN);
    const endIndex = text.lastIndexOf(HANDOFF_PAYLOAD_V3_END);
    if (beginIndex === -1 || endIndex === -1 || endIndex <= beginIndex) {
        throw new ContextDelegationError('PLUGIN_INTERNAL_ERROR', 'Missing or malformed V3 markers in handoff payload.', {
            layer: 'plugin',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
