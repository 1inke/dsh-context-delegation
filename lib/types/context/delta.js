 ...
 */
export function applyContextDelta(base, delta) {
    if (base.revision !== delta.baseRevision) {
        throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Base revision mismatch: base snapshot is ${base.revision} but delta expects ${delta.baseRevision}.`, {
            layer: 'context',
            codexInvoked: false,
    ...
    for (const unit of delta.added) {
        if (addedIds.has(unit.id) || baseMap.has(unit.id)) {
            throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Invalid delta: added unit "${unit.id}" already exists or appears multiple times.`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
        ...
        if (computeContentHash(unit.content) !== unit.contentHash) {
            throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Invalid delta: contentHash mismatch for added unit "${unit.id}".`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
                nextAction: 'Reject delta and trigger FULL_REFRESH.',
    ...
    for (const unit of delta.changed) {
        if (changedIds.has(unit.id) || addedIds.has(unit.id) || !baseMap.has(unit.id)) {
            throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Invalid delta: changed unit "${unit.id}" is missing from base or overlaps with added.`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
                nextAction: 'Reject delta and trigger FULL_REFRESH.',
        ...
        if (computeContentHash(unit.content) !== unit.contentHash) {
            throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Invalid delta: contentHash mismatch for changed unit "${unit.id}".`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
                nextAction: 'Reject delta and trigger FULL_REFRESH.',
    ...
    for (const rem of delta.removed) {
        if (removedIds.has(rem.id) || addedIds.has(rem.id) || changedIds.has(rem.id) || !baseMap.has(rem.id)) {
            throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Invalid delta: removed unit "${rem.id}" is missing from base or overlaps with added/changed.`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
                nextAction: 'Reject delta and trigger FULL_REFRESH.',
    ...
    if (computedRevision !== delta.revision) {
        throw new ContextDelegationError('DELTA_APPLICATION_FAILED', `Target revision mismatch: expected ${delta.revision}, computed ${computedRevision}.`, {
            layer: 'context',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
            nextAction: 'Reject delta and trigger FULL_REFRESH.',
