...
const APPEND_ONLY_TARGETS = new Set(['DECISIONS', 'EXPERIMENTS']);
 ...
 */
export function buildContextPatches(proposal) {
    const patches = [];
    const trustNotes = [];
    ...
    if (projectContext.length > 0) {
        if (overflow(patches.length + 1))
            trustNotes.push('patch limit reached; project-context update dropped');
        else {
...
export async function computeContextFingerprint(workspaceRoot, contextRoot) {
    const parts = [];
...
function assertParsable(content, maxSnapshotBytes) {
    ...
    if (chunks.length === 0)
        throw new Error('patched context file does not parse into chunks');
}
/**
...
export async function applyContextPatches(options) {
    ...
    for (const [target, patches] of byFile) {
        ...
        if (hadTrailingNewline || patchedText.length > 0)
            patchedText += newline;
