import { tokenizeContext } from "./query.js";
...
function includesPath(path, text) {
    ...
    while (offset >= 0) {
        ...
        if (!pathChar.test(before) && !pathChar.test(after))
            return true;
        offset = normalizedText.indexOf(normalizedPath, offset + 1);
    }
    return false;
}
export function rankContextChunks(chunks, query) {
    ...
    for (const directory of directories) {
        for (const term of commonDirectoryTerms)
            if (!directory.has(term))
                commonDirectoryTerms.delete(term);
    }
    ...
    return results.sort((a, b) => b.score - a.score || compareText(a.relativePath, b.relativePath) ||
