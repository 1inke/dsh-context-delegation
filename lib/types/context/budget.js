...
import { DEFAULT_TRUNCATION_NOTICE, } from "../types.js";
import { MAX_CONTEXT_CHARS_HARD_LIMIT } from "../config.js";
/**
 ...
 */
export function budgetContextFiles(candidates, policy) {
    ...
    if (!Number.isSafeInteger(maxChars) || maxChars <= 0 || maxChars > MAX_CONTEXT_CHARS_HARD_LIMIT) {
        throw new ContextDelegationError('PLUGIN_INTERNAL_ERROR', `maxContextChars must be a positive safe integer no greater than ${MAX_CONTEXT_CHARS_HARD_LIMIT}.`, {
            layer: 'plugin',
            codexInvoked: false,
    ...
    for (const c of sorted) {
        if (c.priority === 1 && (!c.sourceComplete || c.content.length > maxChars)) {
            throw new ContextDelegationError('CONTEXT_TOO_LARGE', `AGENTS.md exceeds the maximum context budget (${maxChars}).`, {
                layer: 'context',
                codexInvoked: false,
                workspaceMayHaveChanged: false,
