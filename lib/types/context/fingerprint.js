...
export function computeUnitId(sourceId, relativePath, headingPath, partIndex) {
...
export function computePolicyRevision(options) {
...
export function buildContextSnapshot(options) {
    ...
    // 1. Mandatory files
    const mandatory = bundle.selection?.mandatoryFiles ?? bundle.files.filter(f => ['agents', 'currentState', 'handoffRules'].includes(f.id));
    ...
    if (maxSnapshotBytes !== undefined) {
        ...
        if (serializedBytes > maxSnapshotBytes) {
            throw new ContextDelegationError('SNAPSHOT_TOO_LARGE', `Serialized context snapshot size (${serializedBytes} bytes) exceeds limit (${maxSnapshotBytes} bytes).`, {
                layer: 'context',
                codexInvoked: false,
