...
export function isAbortError(err, signal) {
    ...
    if (err && typeof err === 'object') {
        if ('name' in err && err.name === 'AbortError')
            return true;
        if ('code' in err && err.code === 'FS_ABORTED')
            return true;
    }
    return false;
}
...
export function normalizeRelativePath(rawPath) {
    if (!rawPath.trim()) {
        throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', 'Empty path in relevant_files.', {
            layer: 'context',
            codexInvoked: false,
    ...
    if (rawPath.trim() !== rawPath) {
        throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${rawPath}' in relevant_files must not contain surrounding whitespace.`, {
            layer: 'context',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
            nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
    ...
    if (candidatePath.startsWith('/') || candidatePath.startsWith('\\')) {
        throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${rawPath}' in relevant_files must be a relative path, not absolute.`, {
            layer: 'context',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
            nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
    ...
    if (/^[a-zA-Z]:/.test(candidatePath) || candidatePath.startsWith('\\\\')) {
        throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${rawPath}' in relevant_files contains a drive letter or UNC prefix.`, {
            layer: 'context',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
            nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
    ...
    for (const seg of posix.split('/')) {
        ...
        if (seg === '..') {
            if (stack.length === 0) {
                throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${rawPath}' traverses outside workspace root via parent segment (..).`, {
                    layer: 'context',
                    codexInvoked: false,
                    workspaceMayHaveChanged: false,
                    nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
    ...
    if (stack.length === 0) {
        throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${rawPath}' resolves to workspace root itself, not a valid target file.`, {
            layer: 'context',
            codexInvoked: false,
            workspaceMayHaveChanged: false,
            nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
...
export async function resolveWorkspaceTarget(fs, workspaceRoot, signal) {
    try {
        const target = await fs.resolve('.', { cwd: workspaceRoot, signal });
        const info = await fs.stat(target, signal);
        if (!info)
            throw workspaceError(workspaceRoot, 'the path does not exist');
        if (info.type !== 'directory')
            throw workspaceError(workspaceRoot, `expected a directory but found '${info.type}'`);
        return target;
    }
    catch (err) {
        ...
        if (isAbortError(err, signal))
            throw cancelledError({ cause: err });
 ...
 */
export async function validateRelevantFiles(request) {
    ...
    try {
        ...
        for (const raw of relevantFiles) {
            ...
            if (!isContained) {
                throw new ContextDelegationError('CONTEXT_PATH_ESCAPE', `Path '${normalized}' resolves outside workspaceRoot (${workspaceRoot}).`, {
                    layer: 'context',
                    codexInvoked: false,
                    workspaceMayHaveChanged: false,
                    nextAction: 'Ensure all context and relevant files reside strictly within the workspace root.',
            ...
            if (info !== undefined) {
                if (info.type === 'directory') {
                    throw new ContextDelegationError('CONTEXT_READ_ERROR', `relevant_files path '${normalized}' is a directory, not a file.`, {
                        layer: 'context',
                        codexInvoked: false,
                        workspaceMayHaveChanged: false,
                ...
                if (info.type === 'other') {
                    throw new ContextDelegationError('CONTEXT_READ_ERROR', `relevant_files path '${normalized}' is a special non-regular file (${info.type}).`, {
                        layer: 'context',
                        codexInvoked: false,
                        workspaceMayHaveChanged: false,
                        nextAction: 'Verify file readability, permissions, and ensure context paths are regular files.',
...
//# sourceMappingURL=path-confinement.js.map