...
function addToken(tokens, seen, token) {
    ...
    if (!value || ENGLISH_STOPWORDS.has(value) || CHINESE_STOPWORDS.has(value) || seen.has(value))
        return;
    seen.add(value);
    tokens.push(value);
}
...
export function tokenizeContext(text) {
    ...
    for (const run of runs) {
        ...
        for (const piece of pieces) {
            ...
            for (const subpiece of piece.split(/[_-]+/u))
                addToken(tokens, seen, subpiece);
        }
        ...
        for (const sequence of chinese) {
            ...
            for (const char of chars)
                addToken(tokens, seen, char);
            for (let i = 0; i + 1 < chars.length; i++)
                addToken(tokens, seen, chars[i] + chars[i + 1]);
        }
    }
    return tokens;
}
export function buildContextQuery(input, relevantFiles = input.relevant_files ?? []) {
