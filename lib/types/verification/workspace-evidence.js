...
export function classifyGitError(error, fallbackReason = 'git command failed') {
...
function runGit(args, workspaceRoot, signal) {
    return new Promise((resolvePromise, rejectPromise) => {
        ...
        execFile('git', ['-c', 'safe.directory=*', '-c', 'core.quotepath=false', ...args], {
            cwd: workspaceRoot,
            windowsHide: true,
...
function parseChangedFiles(porcelain) {
    ...
    for (const line of porcelain.split('\n')) {
        ...
        if (path.length > 0)
            files.push(normalizeGitPath(path));
...
function extractDiffSummary(diffStat) {
    ...
    if (summary.length === 0 || !/changed/i.test(summary))
        return undefined;
 ...
 */
export async function collectWorkspaceEvidence(options) {
...
//# sourceMappingURL=workspace-evidence.js.map