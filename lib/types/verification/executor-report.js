...
const EXECUTOR_STATUSES = ['completed', 'blocked', 'failed'];
const CLAIM_OUTCOMES = ['passed', 'failed', 'not-run'];
...
function coerceContextUpdate(value) {
    ...
    const decisions = Array.isArray(raw.decisions)
        ? raw.decisions.slice(0, 8).map((entry) => {
            const item = (entry ?? {});
    ...
    const experiments = Array.isArray(raw.experiments)
        ? raw.experiments.slice(0, 8).map((entry) => {
            const item = (entry ?? {});
            return {
...
function coerceClaim(value) {
    const raw = (value ?? {});
    const outcome = typeof raw.outcome === 'string' && CLAIM_OUTCOMES.includes(raw.outcome)
        ? raw.outcome
        : 'not-run';
    return {
 ...
 */
export function parseExecutorReport(finalText) {
    ...
    if (begin >= 0) {
        ...
        if (jsonStart >= 0 && end > jsonStart) {
            ...
            try {
                ...
                const status = typeof raw.status === 'string' && EXECUTOR_STATUSES.includes(raw.status)
                    ? raw.status
                    : undefined;
...
//# sourceMappingURL=executor-report.js.map