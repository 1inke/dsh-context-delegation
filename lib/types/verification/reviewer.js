...
const VERDICTS = ['pass', 'rework', 'blocked'];
 ...
 * from data fields.
 */
export function buildReviewerPrompt(request) {
...
function coerceReport(raw) {
    const verdict = typeof raw.verdict === 'string' && VERDICTS.includes(raw.verdict)
        ? raw.verdict
        : undefined;
...
export function parseReviewReport(finalText) {
    ...
    const candidates = [];
    for (const match of finalText.matchAll(fencePattern))
