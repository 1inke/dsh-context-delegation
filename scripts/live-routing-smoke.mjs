// Opt-in real-runtime routing smoke (V0.5 evidence).
//
// Difference from live-review-smoke.mjs: the SAME spawn provider class is
// registered twice under two DISTINCT provider names, and executorRouting
// routes implementation and reviewer to different executor ids. Success of
// the reviewed loop therefore proves the routing layer resolved and drove
// two separately-registered executors.
//
...
try {
  ...
  const Driver = {
    ...
    async apply(driverCtx) {
      const service = new ContextDelegationService(driverCtx, {
        ...
        maxReviewRounds: 1,
        executorRouting: {
          implementation: { provider: 'native-impl' },
          reviewer: { provider: 'spawn-reviewer' },
        },
      })
  ...
  for (const entry of await readdir(root, { recursive: true, withFileTypes: true })) {
    if (entry.isFile() && !entry.name.endsWith('.output')) {
      const path = join(entry.parentPath ?? entry.path, entry.name)
      if (!baselineHashes.has(path) && !path.endsWith('answer.txt')) report.baselineFilesUnchanged = false
    }
  }
  const review = report.review ?? {}
  const checks = {
    reviewOutcomeCompleted: review.outcome === 'completed',
    routedProviderUsed: report.routedImplementationProvider === 'native-impl',
